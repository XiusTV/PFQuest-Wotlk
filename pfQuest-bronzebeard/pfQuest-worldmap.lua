...
-- Get zone data from either main or bronzebeard databases
local function GetZoneData(zoneID)
    local zoneData = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and pfDB["zones"]["data"][zoneID]
    if not zoneData then
        zoneData = pfDB and pfDB["zones"] and pfDB["zones"]["data-bronzebeard"] and pfDB["zones"]["data-bronzebeard"][zoneID]
    end
    return zoneData
end
                                ...
                                for title, data in pairs(node) do
                                    ...
                                    if data.addon == "PFDB" then
                                        ...
                                        for _, utilityType in pairs(utilityTypes) do
                                            if pfDB["meta-bronzebeard"] and pfDB["meta-bronzebeard"][utilityType] then
                                                for objectId, faction in pairs(pfDB["meta-bronzebeard"][utilityType]) do
                                        ...
                                        if not isUtilityNPC then
                                            ...
                                            for _, blockedType in pairs(blockedTypes) do
                                                if pfDB["meta-bronzebeard"] and pfDB["meta-bronzebeard"][blockedType] then
                                                    for objectId, faction in pairs(pfDB["meta-bronzebeard"][blockedType]) do
                                                        if data.id and tonumber(data.id) == objectId then
                                                            skipNode = true
                                                            break
                                                        end
                                        ...
                                        if worldX and worldY then
                                            ...
                                            if
                                             ...
                                             then
                                                ...
                                                -- NOTE: This is where you fix the overworld map offsets 0.85 is the scale and the + or - numbers are the x and y offsets
                                                if targetContinent == 1 then
                                                    worldMapX = contX * 0.85 - 0.22
                                                    worldMapY = contY * 0.85 + 0.17
                                                else
                                                    worldMapX = 0.39 + (contX * 0.85)
                                                    worldMapY = contY * 0.85 + 0.115
                                                end
...
local function ExtendPfQuestConfig()
    ...
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "1",
            type = "checkbox",
            config = "bronzebeardContinentPins"
        }
    )
    table.insert(
        pfQuest_defconfig,
    ...
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "1",
            type = "checkbox",
            config = "bronzebeardHideChickenQuests"
        }
    )
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "1",
            type = "checkbox",
            config = "bronzebeardHideFelwoodFlowers"
        }
    )
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "1",
            type = "checkbox",
            config = "bronzebeardHidePvPQuests"
        }
    )
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "0",
            type = "checkbox",
            config = "bronzebeardHideCommissionQuests"
        }
    )
    table.insert(
        pfQuest_defconfig,
        {
            ...
            default = "0",
            type = "checkbox",
            config = "bronzebeardHideItemDrops"
        }
    )

    -- Initialize the config values with defaults
    pfQuest_config["bronzebeardContinentPins"] = pfQuest_config["bronzebeardContinentPins"] or "1"
    ...
    pfQuest_config["bronzebeardHideChickenQuests"] = pfQuest_config["bronzebeardHideChickenQuests"] or "1"
    pfQuest_config["bronzebeardHideFelwoodFlowers"] = pfQuest_config["bronzebeardHideFelwoodFlowers"] or "1"
