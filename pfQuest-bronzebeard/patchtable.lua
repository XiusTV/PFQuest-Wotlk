...
for _, db in pairs(dbs) do
  if pfDB[db]["data-bronzebeard"] then
    patchtable(pfDB[db]["data"], pfDB[db]["data-bronzebeard"])
  end

  for loc, _ in pairs(pfDB.locales) do
    if pfDB[db][loc] and pfDB[db][loc.."-bronzebeard"] then
      loc_update = pfDB[db][loc.."-bronzebeard"] or pfDB[db]["enUS-bronzebeard"]
...
loc_update = pfDB["professions"][loc.."-bronzebeard"] or pfDB["professions"]["enUS-bronzebeard"]
if loc_update then patchtable(loc_core, loc_update) end

if pfDB["minimap-bronzebeard"] then patchtable(pfDB["minimap"], pfDB["minimap-bronzebeard"]) end
if pfDB["meta-bronzebeard"] then patchtable(pfDB["meta"], pfDB["meta-bronzebeard"]) end
...
pfQuest.dburl = "https://db.ascension.gg/?quest="
...
updatecheck:SetScript("OnEvent", function()
  if pfDB["quests"]["data-bronzebeard"] then
    -- count all known bronzebeard quests
    local count = 0
    for k, v in pairs(pfDB["quests"]["data-bronzebeard"]) do
      count = count + 1
    end

    pfQuest:Debug("Bronzebeard loaded with |cff33ffcc" .. count .. "|r quests.")

    -- check if the last count differs to the current amount of quests
    if not pfQuest_bronzebeardcount or pfQuest_bronzebeardcount ~= count then
    ...
    pfQuest_bronzebeardcount = count
  end
end)
...
pfMap.NodeEnter = function(self)
  ...
  for title, meta in pairs(self.node) do
  ...
  if hasItemStart and itemStartMeta then
    ...
    self.spawn = self.spawn or UNKNOWN
    tooltip:SetText(self.spawn..(pfQuest_config.showids == "1" and " |cffcccccc("..self.spawnid..")|r" or ""), .3, 1, .8)
    tooltip:AddDoubleLine(pfQuest_Loc["Type"] .. ":", (self.spawntype or UNKNOWN), .8,.8,.8, 1,1,1)
